-- =======================================================
-- BỘ 15 CÂU TRUY VẤN THỐNG KÊ QUẢN LÝ SIÊU THỊ
-- =======================================================
...
-- Câu 1: Xem danh sách tất cả sản phẩm hiện có trong kho, sắp xếp theo tên A-Z
SELECT * FROM mat_hang ORDER BY ten ASC;

-- Câu 2: Cảnh báo hàng tồn kho thấp (Tồn kho dưới 10) để kịp thời nhập thêm
SELECT id, ten, ton_kho FROM mat_hang WHERE ton_kho < 10;

-- Câu 3: Lọc ra các sản phẩm cao cấp có giá bán lớn hơn 100.000 VNĐ
SELECT ten, gia_ban FROM mat_hang WHERE gia_ban > 100000;

-- Câu 4: Tìm kiếm nhanh các mặt hàng theo từ khóa (Ví dụ: Tìm tất cả các loại "Sữa")
SELECT * FROM mat_hang WHERE ten LIKE N'%Sữa%';

-- Câu 5: Tính trung bình cộng giá bán của tất cả các mặt hàng đang kinh doanh
...
-- Câu 6: Tính tổng doanh thu thu được từ tất cả các hóa đơn bán ra
SELECT SUM(thanh_tien) AS TongDoanhThu FROM hoa_don_xuat;

-- Câu 7: Tính tổng chi phí vốn đã bỏ ra để nhập hàng về kho
SELECT SUM(thanh_tien) AS TongChiPhiNhapHang FROM hoa_don_nhap;

-- Câu 8: Tính toán mức biên độ lợi nhuận trên từng đơn vị sản phẩm (Giá bán - Giá nhập)
SELECT ten AS TenSanPham, gia_nhap, gia_ban, (gia_ban - gia_nhap) AS LoiNhuanTren1SP 
FROM mat_hang;

-- Câu 9: Trích xuất 5 hóa đơn bán hàng có giá trị cao nhất (Khách VIP)
...
-- Câu 10: Thống kê KPI - Đếm số lượng hóa đơn mỗi nhân viên thu ngân đã xử lý
SELECT nhan_vien, COUNT(id) AS SoDonDaBan 
FROM hoa_don_xuat 
GROUP BY nhan_vien;

-- Câu 11: Tra cứu thông tin của các nhân viên Nam đang giữ chức vụ 'Quản lý'
SELECT ten, sdt FROM nhan_vien 
WHERE gioi_tinh = N'Nam' AND chuc_vu = N'Quản lý';

-- Câu 12: Tìm kiếm các Nhà cung cấp có trụ sở tại TP HCM và số điện thoại đuôi '22'
